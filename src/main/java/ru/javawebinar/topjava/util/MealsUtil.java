...
//    }
    ...
    public static List<MealTo> filterByPredicate(Collection<Meal> meals, int caloriesPerDay, Predicate<Meal> filter) {
        ...
        return meals.stream()
                .filter(filter)
                .map(meal -> createTo(meal, caloriesSumByDate.get(meal.getDate()) > SecurityUtil.authUserCaloriesPerDay())).sorted(mealToComparator)
                .collect(Collectors.toList());
    }


    private static MealTo createTo(Meal meal, boolean excess) {
        return new MealTo(meal.getId(), meal.getDateTime(), meal.getDescription(), meal.getCalories(), SecurityUtil.authUserId(), excess);
    }

    private static Comparator<MealTo> mealToComparator = (MealTo o1, MealTo o2) -> {
        if (o1.getDateTime().isAfter(o2.getDateTime())) return 1;
        else if (o1.getDateTime().equals(o2.getDateTime())) return 0;
        return -1;
    };
